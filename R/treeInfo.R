...
treeInfo <- function(object, tree = 1) {
  ...
  if (forest$treetype == "Classification") {
    ...
    result$prediction <- forest$split.values[[tree]]
    result$prediction[!result$terminal] <- NA
    result$n <- forest$n.values[[tree]] # Added by Nikola Surjanovic
    result$sum.y2 <- forest$y2.values[[tree]] # Added by Nikola Surjanovic
    result$mean.y <- forest$meany.values[[tree]] # Added by Nikola Surjanovic
    result$sse <- result$sum.y2 - result$n*(result$mean.y)^2 # Added by Nikola Surjanovic
