...
ranger <- function(formula = NULL, data = NULL, num.trees = 500, mtry = NULL,
  ...
  # Added by Nikola Surjanovic -----------------
  factor.levels <- vector(mode='list', length=ncol(data))
  names(factor.levels) <- colnames(data)
  
  for (j in 1:ncol(data)) {
    if (is.factor(data[, j])) {
      factor.levels[[j]] <- levels(data[, j])
      data[, j] <- as.numeric(data[, j])
    }
  }
  result$data <- as.matrix(data)
  result$factor.levels <- factor.levels
  # End addition by Nikola Surjanovic ----------
