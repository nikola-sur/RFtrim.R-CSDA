...
#include <ctime>
#include <cmath> // Added by Nikola Surjanovic
...
double TreeRegression::sumy2(size_t nodeID) { // Added by Nikola Surjanovic

// Sum of squared responses of samples in node
  double sum_sq_responses_in_node = 0;
  for (size_t pos = start_pos[nodeID]; pos < end_pos[nodeID]; ++pos) {
    size_t sampleID = sampleIDs[pos];
    sum_sq_responses_in_node += std::pow(data->get_y(sampleID, 0),2);
  }
  return (sum_sq_responses_in_node);
}
...
bool TreeRegression::splitNodeInternal(size_t nodeID, std::vector<size_t>& possible_split_varIDs) {
  ...
  // Added by Nikola Surjanovic: Update n_values
  n_values[nodeID] = num_samples_node;

  // Added by Nikola Surjanovic: Update y2_values
  y2_values[nodeID] = sumy2(nodeID);
  
  // Added by Nikola Surjanovic: Update meany_values
  meany_values[nodeID] = estimate(nodeID);
