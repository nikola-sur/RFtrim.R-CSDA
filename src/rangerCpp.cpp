...
Rcpp::List rangerCpp(uint treetype, Rcpp::NumericMatrix& input_x, Rcpp::NumericMatrix& input_y,
  ...
  try {
    ...
    if (write_forest) {
      ...
      forest_object.push_back(forest->getNValues(), "n.values"); // Added by Nikola Surjanovic
      forest_object.push_back(forest->getY2Values(), "y2.values"); // Added by Nikola Surjanovic
      forest_object.push_back(forest->getMeanYValues(), "meany.values"); // Added by Nikola Surjanovic
